...
この例では、`BlogPosting`サービスを使用して[Blogs]ウィジェットからブログ投稿を取得していますが、この手順は公開されているどのサービスでも使用できます。
...
API検出により、ブログ投稿サービスが`headless-delivery`アプリケーションで利用可能であることが明らかになりました。

1.  `headless-delivery`を照会して`BlogPosting`スキーマを検索します。

    ``` bash
    curl "http://localhost:8080/o/headless-delivery/v1.0/openapi.yaml" -u test@liferay.com:test
    ```

    これにより、（数ある中で）`BlogPosting`のスキーマが明らかになります。 必須フィールドは2つだけです。
